...
class DATAREPOSITORY_API FlintRasterReaderFactory : public RasterReaderFactoryInterface {
  public:
   ...
   bool hasStackRasterReader() const override { return true; }

   std::unique_ptr<MetaDataRasterReaderInterface> getMetaDataRasterReader(const std::string& path,
                                                                          const std::string& prefix,
                                                                          const DynamicObject& settings) const override;

   std::unique_ptr<TileRasterReaderInterface> getTileRasterReader(const std::string& path, const Point& origin,
                                                                  const std::string& prefix,
                                                                  const TileBlockCellIndexer& indexer,
                                                                  const DynamicObject& settings) const override;

   std::unique_ptr<StackRasterReaderInterface> getStackRasterReader(const std::string& path, const Point& origin,
                                                                    const std::string& prefix,
                                                                    const TileBlockCellIndexer& indexer,
                                                                    const DynamicObject& settings) const override;
};
